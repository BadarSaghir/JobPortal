...
{
    if (!string.IsNullOrWhiteSpace(cert.CertificateName) && string.IsNullOrWhiteSpace(cert.IssuingBody))
        cert.IssuingBody = "NONE";
}
    if (model.DocumentAttachments != null)
    {
        foreach (var entry in model.DocumentAttachments)
        {
            foreach (var file in entry.Files)
            {
                if (file.Length > 150 * 1024) // 150 KB
                {
                    ModelState.AddModelError("", $"The file '{file.FileName}' exceeds the 150KB limit. Make sure to upload file again!");
                }
            }
        }
    }
      ...
      if (!ModelState.IsValid) 
        {
            ModelState.AddModelError("", "Please correct the highlighted errors. Note: You must re-select your file uploads.");
