...
public async Task<IActionResult> Edit(Guid id)
{
    ...
    //   model.PostedAt = job.PostedAt.ToLocalTime();
    // model.ExpiresAt = job.ExpiresAt.ToLocalTime();
    model.RequiredSkillNames=job.RequiredSkills.Select(s => s.SkillName).ToList();
    model.PostedAt = job.PostedAt; 
model.ExpiresAt = job.ExpiresAt;
...
public async Task<IActionResult> Edit(Guid id, EditJobViewModel model)
{
    ...
    try 
    {
        await _commandProcessor.SendAsync(new UpdateJobCommand(jobData, model.RequiredSkillNames));
        return RedirectToAction(nameof(Index));
    }
    catch (Exception ex)
    {
