...
public async Task<IActionResult> MarkAllAsRead()
{
    ...
    await commandProcessor.SendAsync(new MarkAllAsReadCommand(userId));

    TempData["Success"] = "All alerts have been cleared and marked as read.";
    
    // Redirect back to the notification registry
    return RedirectToAction(nameof(Notifications));
}
// Areas/Admin/Controllers/AccountAdminController.cs
...
public async Task<IActionResult> NotificationDetails(Guid id)
{
    ...
    // 1. Fetch Details
    var notification = await queryProcessor.ExecuteAsync(new GetNotificationDetailQuery { 
        NotificationId = id, 
        UserId = userId 
    });
    ...
    // 2. Mark as read automatically
    if (!notification.IsRead)
    {
        await commandProcessor.SendAsync(new MarkAsReadCommand(id));
    }
